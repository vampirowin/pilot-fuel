...
def _render_critical_groups(sorted_groups: list, vmap: dict) -> str:
    ...
    class_map = {'normal': 'status-normal', 'small_deviation': 'status-small-deviation', 'unacceptable': 'status-unacceptable', 'pilot_missing': 'status-pilot-missing', 'check_missing': 'status-check-missing', 'false_reading': 'status-false-reading'}
    label_map = {'normal': 'Норма', 'small_deviation': 'Расхождение', 'unacceptable': 'Недопустимо', 'pilot_missing': 'Нет в Pilot', 'check_missing': 'Нет чека', 'false_reading': 'Ложная'}
