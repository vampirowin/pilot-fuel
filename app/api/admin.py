...
def render_no_sensor_partial(vehicles: list) -> str:
    ...
    for group_name, group_vehicles in groups:
        ...
        html += f'<div class="card level-folder" style="margin-top: 16px;"><div class="card-header collapsible-header" onclick="toggleGroup(\'{gid}\')"><span class="arrow">&#9660;</span><div class="level-header-center"><span class="level-title">{group_name}</span><span class="level-tag">Тип ТС</span></div><span class="level-count">{len(group_vehicles)}</span></div><div class="collapsible-body" id="{gid}"><div class="table-container"><table><thead><tr><th style="width:32px;"><input type="checkbox" onchange="{select_all_js}"></th><th>#</th><th>Госномер</th><th>IMEI</th><th>Действия</th></tr></thead><tbody>'
